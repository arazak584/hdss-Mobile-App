...
import org.openhds.hdsscapture.databinding.FragmentSocioDBinding;
...
import org.openhds.hdsscapture.fragment.SocioCFragment;
import org.openhds.hdsscapture.fragment.SocioEFragment;
 ...
 * Use the {@link SocioDViewFragment#newInstance} factory method to
 * create an instance of this fragment.
 */
public class SocioDViewFragment extends KeyboardFragment {


    private static final String SOCIAL_ID = "SOCIAL_ID";
    private FragmentSocioDBinding binding;
    private HdssSociodemo hdssSociodemo;
    private HdssSociodemoRepository repository;

    public SocioDViewFragment() {
     ...
     */
    //
    public static SocioDViewFragment newInstance(String uuid) {
        SocioDViewFragment fragment = new SocioDViewFragment();
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        binding = FragmentSocioDBinding.inflate(inflater, container, false);
        ...
        loadCodeData(binding.petVac, "pet_vac");
        loadCodeData(binding.id00061Option, "crop");
        loadCodeData(binding.id00071Option, "crop");
        loadCodeData(binding.id00081Option, "crop");
        loadCodeData(binding.id00091Option, "crop");
        loadCodeData(binding.id00101Option, "crop");
        loadCodeData(binding.id00111Option, "crop");
        loadCodeData(binding.id00111Option, "crop");
        loadCodeData(binding.id00121Option, "crop");
        loadCodeData(binding.id00131Option, "crop");
        loadCodeData(binding.id00141Option, "crop");
        loadCodeData(binding.id00151Option, "crop");
        loadCodeData(binding.id00161Option, "crop");
        loadCodeData(binding.id00171Option, "crop");
        loadCodeData(binding.id00181Option, "crop");
        loadCodeData(binding.id00191Option, "crop");
    ...
    private void save(boolean save, boolean close, HdssSociodemoViewModel viewModel) {
        ...
        if (save) {
            requireActivity().getSupportFragmentManager().beginTransaction().replace(R.id.fragment_container,
                    SocioEViewFragment.newInstance(hdssSociodemo.uuid)).commit();

        }else {
            requireActivity().getSupportFragmentManager().beginTransaction().replace(R.id.fragment_container,
                    SocioCViewFragment.newInstance(hdssSociodemo.uuid)).commit();
        }
