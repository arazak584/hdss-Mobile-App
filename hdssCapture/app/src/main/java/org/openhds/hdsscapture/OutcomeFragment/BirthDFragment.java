 ...
 * Use the {@link BirthDFragment#newInstance} factory method to
 * create an instance of this fragment.
 */
public class BirthDFragment extends Fragment {
    ...
    public static BirthDFragment newInstance(Individual individual, Locations locations, Socialgroup socialgroup) {
        BirthDFragment fragment = new BirthDFragment();
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        try {
            Pregnancyoutcome datas = viewModel.findloc(selectedIndividual.uuid, selectedLocation.compno);
            if (datas != null) {
                binding.setPregoutcome(datas);

                try {
                    final String child_id = selectedIndividual.uuid + AppConstants.CHILD4 + 0 + roundData.roundNumber;
    ...
    private void save(boolean save, boolean close, OutcomeViewModel outcomeViewModel, IndividualViewModel individualViewModel, ResidencyViewModel residencyViewModel) {
        ...
        if (save) {
            requireActivity().getSupportFragmentManager().beginTransaction().replace(R.id.container_cluster,
                    BirthSFragment.newInstance(individual, locations, socialgroup)).commit();
        }else {
            requireActivity().getSupportFragmentManager().beginTransaction().replace(R.id.container_cluster,
                    BirthCFragment.newInstance(individual, locations, socialgroup)).commit();
        }
